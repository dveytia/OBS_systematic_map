...
```{r connect to database with the keyword matches and load associated text}
## Connect to database
keywordCon <- RSQLite::dbConnect(RSQLite::SQLite(), 
                                 dbname = here::here("data","derived-data","coding","keyword-matches","keyword-matches.sqlite"))
src_dbi(keywordCon)
...
keywordMatches <- tbl(keywordCon, "keywordMatches")
keywordMatches %>%head %>% collect() %>% View
keywordMatches %>%head %>% show_query()
...
```{r For each paper join keyword matches with metadata and }
