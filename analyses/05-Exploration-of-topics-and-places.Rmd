...
ggp_list[[PaperNumber]] <- paper_tab %>%
  ...
  ggplot(aes(x=Topic, y=n, fill = `Group name`)) + 
    geom_col()+
    geom_text(aes(y = ymax+yoffset, label = paste("total papers = ", paper_total)))+
    labs(
...
ggp_list[[PaperNumber]] <- counts %>%
  ...
  ggplot(aes(x=Topic, y=n, fill = `Group name`)) + 
    geom_col()+
    geom_text(aes(x=paste("Paper", PaperNumber), y = n, label=paste(n),group = `Group name`), position = position_stack(vjust=0.5))+
    #geom_text(aes(x=paste("Paper", PaperNumber), y = ymax+yoffset, label = paste("total papers = ", paper_total)))+
    labs(
      title = paste(strwrap(tempTitle, width = 40), collapse="\n"),
      y=yaxis_title
...
# for each group of 'columns' format at T/F if there was a match or not
