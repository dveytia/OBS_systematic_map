...
dedups <- tbl(all_db, "uniquerefs") %>% filter(!is.na(abstract))  # unique references 
...
# Summarise predicted relevance -- the numbers don't change
keepIDs <- dedups %>% select(analysis_id)%>%collect() # unique ids   
keepIDs <- unique(keepIDs$analysis_id)

predRel <- predRel[predRel$analysis_id %in% keepIDs,]
