...
Firstly, how many articles are predicted to be relevant for each ORO branch? Does this differ depending on whether the mean or upper estimate is used?
```{r tabulate oro_branch}
predBranch[,grep("mean", colnames(predBranch))]%>%
  reshape2::melt(variable.name = "ORO_branch") %>% 
  group_by(ORO_branch)%>%
  summarise(n_articles = sum(0.5 <= value))

predBranch[,grep("upper", colnames(predBranch))]%>%
  reshape2::melt(variable.name = "ORO_branch") %>% 
  group_by(ORO_branch)%>%
  summarise(n_articles = sum(0.5 <= value))
```
...
Of the articles predicted to be relevant (0.5 <= mean estimate) what are the counts for each ORO branch?

```{r of the articles predicted to be relevant what branch are they classified in}
# using mean
print("using mean estimate")
df[c(grep("mean", colnames(df)), which(colnames(df) == "Unclear"))] %>%
  filter(0.5 <= relevance_mean) %>%
  select(-c(relevance_mean))%>%
  reshape2::melt(variable.name = "ORO_branch") %>%
  group_by(ORO_branch)%>% 
  summarise(n_articles = sum(0.5 <= value, na.rm=T))


# using mean
print("using upper estimate")
df[grep("upper", colnames(df))] %>%
  filter(0.5 <= relevance_upper) %>%
  select(-c(relevance_upper))%>%
  reshape2::melt(variable.name = "ORO_branch") %>%
  group_by(ORO_branch)%>% 
  summarise(n_articles = sum(0.5 <= value, na.rm=T))
...
rm(dedups)
predOROAnyMeta <- predOROAnyMeta %>% full_join(screens2, by = "duplicate_id")
...
predOROAnyMeta <- dedups %>%
  inner_join(predOROAny, by = "analysis_id") %>%
  collect()
predRelMeta <- merge(predRelMeta, screens2, all.y=TRUE)
