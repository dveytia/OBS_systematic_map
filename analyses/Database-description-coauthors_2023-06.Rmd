...
These inter-branch interactions occur most often between Conservation options (Protection and Restoration) and CO2 removal and storage, as well as with societal adaptation options. However, recalling evaluating the predictions for ORO type against the known values, both mitigation and societal branches tended to also be predicted as relevant for Natural resilience options. So it is difficult to discern if this is a true phenomenon or just a result of the overlapping language and objectives across these options. It is likely the latter, as objectives for both mitigation and societal adaptation can often motivate nature-based solutions and adaptation options. 
...
While neither of these variables received good F1 scores, we can still compare the predicted relevance with the coded answers.
```{r out.width="80%", fig.asp = 0.6, fig.align='center'}
...
Which "threshold" would yield the best F1 ratio (harmonic mean of precision and recall)? Note that these F1 ratios do not reflect the "true" values obtained through model selection, because these were calculated from all the seen data, rather than across different folds.
...
for(i in 1:length(thresholds)){
  temp <- df %>%
    ...
    select(variable, threshold, F1)
  
  temp$n_articles_upper <- c(
    sum(thresholds[i] <= predPaper2$biodiversity_metric_upper),
    sum(thresholds[i] <= predPaper2$impact_nature_upper)
  )
...
p1 <- ggplot(thresholdsDf, aes(threshold, F1))+
...
p2 <- ggplot(thresholdsDf, aes(threshold, n_articles_upper))+
  facet_wrap(vars(variable))+
  geom_point()+
  geom_vline(data=maxVals,aes(xintercept=threshold),col="red")+
  labs(x="Threshold",y="Relevant articles (N)")+
  theme_bw()

egg::ggarrange(plots = list(p1,p2), nrow=2)

```

If we then apply these thresholds, how many articles are predicted relevant across the entire corpus? Note here the thresholds are rounded to the nearest digit. I.e. 0.73 -> 0.7.

```{r If we then apply these thresholds how many articles relevant in total}
paper2Tab <- predPaper2%>%
  summarise(
    biodiversity_metric_relevant = sum(
      round(maxVals$threshold[maxVals$variable=="biodiversity_metric"], digits = 1) <= biodiversity_metric_upper),
    impact_nature_relevant = sum(
      round(maxVals$threshold[maxVals$variable=="impact_nature"], digits = 1) <= impact_nature_upper)
  )%>%
  reshape2::melt(value.name = "n_articles_upper")%>%
  mutate(
    threshold = c(round(maxVals$threshold[maxVals$variable=="biodiversity_metric"], digits = 1),
                  round(maxVals$threshold[maxVals$variable=="impact_nature"], digits = 1))
  )%>%
  select(variable, threshold, n_articles_upper)


knitr::kable(paper2Tab)
...
```{r read in restoration}

## READ IN
# to speed this up only look at upper predictions

predRestor <- readr::read_csv(
  here::here("data/raw-data/coding-predictions/restoration_predictions.csv"),
  show_col_types = FALSE
)
predRestor <- predRestor[,c(1,grep("upper", colnames(predRestor)))]
colnames(predRestor) <- c("analysis_id",paste("restoration",c("upper"), sep="_"))

```

Compare the predicted relevance with the coded answers.
```{r out.width="60%", fig.asp = 0.6, fig.align='center'}
df <- codebookCombinedSimplifiedMore %>%
  select(analysis_id, restoration)%>%
  reshape2::melt(id.vars = "analysis_id", variable.name = "variable",
                 value.name = "seen_value")%>%
  left_join(predRestor %>%
              rename(restoration = restoration_upper) %>%
              reshape2::melt(id.vars="analysis_id",variable.name = "variable",
                             value.name = "predicted_relevance"))%>%
  filter(!is.na(predicted_relevance)) %>% 
  mutate(seen_value = factor(seen_value, levels = c(0,1), 
...
ggplot(df, aes(x=seen_value, y=predicted_relevance, fill=seen_value, col=seen_value))+
  geom_violin(trim=TRUE, position="dodge")+
  geom_hline(yintercept = 0.5)+
  scale_y_continuous(limits = c(0,1))+
  scale_fill_discrete(guide="none")+
  scale_color_discrete(guide="none")+
  labs(y = "Predicted relevance", x="Reviewer decision")+
  theme_bw()
...
predFish <- predFish[,c(1,grep("upper", colnames(predFish)))]
colnames(predFish) <- c("analysis_id",paste("safe_fish",c("upper"), sep="_"))
...
predSpace <- predSpace[,c(1,grep("upper", colnames(predSpace)))]
colnames(predSpace) <- c("analysis_id",paste("safe_space",c("upper"), sep="_"))


## Join datasets together
predPaper4 <- merge(predFish, predSpace,by="analysis_id")

predPaper4.2 <- merge(predPaper4, 
                      predBranch[,c("analysis_id","Nature_upper", "Societal_upper")],
                      by="analysis_id")
...
```{r out.width="60%", fig.asp = 0.6, fig.align='center'}
df <- codebookCombinedSimplifiedMore %>%
  select(analysis_id, safe_fish, safe_space)%>%
  reshape2::melt(id.vars = "analysis_id", variable.name = "variable",
                 value.name = "seen_value")%>%
  left_join(predPaper4 %>%
              rename(safe_fish = safe_fish_upper,
                     safe_space = safe_space_upper) %>%
              reshape2::melt(id.vars="analysis_id",variable.name = "variable",
                             value.name = "predicted_relevance"))%>%
  filter(!is.na(predicted_relevance)) %>% 
  mutate(seen_value = factor(seen_value, levels = c(0,1), 
...
```{r how many articles are societal AND safe fish or safe space}
predPaper4.2 %>%
  filter(0.5 <= Societal_upper) %>%
  summarise(
    n_safe_fish = sum(0.5 <= safe_fish_upper),
    n_safe_space = sum(0.5 <= safe_space_upper),
    n_both = sum(0.5 <= safe_fish_upper |
                   0.5 <= safe_space_upper)
  )

predPaper4.2 %>%
  filter(0.5 <= Nature_upper) %>%
  summarise(
    n_safe_fish = sum(0.5 <= safe_fish_upper),
    n_safe_space = sum(0.5 <= safe_space_upper),
    n_both = sum(0.5 <= safe_fish_upper |
...
# tabulate values for paper 2
paper2Tab <- paper2Tab %>% 
  select(variable, n_articles_upper) %>%
  mutate(label ="include") %>%
  select(variable, label, n_articles_upper)
paper2Tab$variable <- gsub("_relevant","",paper2Tab$variable)
  

# tabulate restoration
predRestorTab <- predRestor%>%
  summarise(n_articles_upper = sum(0.5 <= restoration_upper))%>%
  mutate(label = paste("include"), variable = paste("restoration"))%>%
  select(variable, label, n_articles_upper)

# tabulate safe fish and safe space
paper4Tab <- predPaper4%>%
  rename(safe_fish = safe_fish_upper, safe_space = safe_space_upper)%>%
  reshape2::melt(id.vars="analysis_id") %>%
  group_by(variable)%>%
  summarise(n_articles_upper = sum(0.5 <= value))%>%
  mutate(label = "include")%>%
  select(variable, label, n_articles_upper)
...
# compbine a dataset of all the upper predictions
upperPred <- predRel %>% 
  ...
  left_join(predPaper2, by="analysis_id") %>%
  left_join(predRestor, by="analysis_id") %>%
  left_join(predPaper4, by="analysis_id")
