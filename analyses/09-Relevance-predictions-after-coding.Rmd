...
predRel <- read.csv(here::here("data","raw-data","relevance-predictions","1_document_relevance_06062023.csv"))
...
sum(duplicated(predRel$analysis_id))
...
dbWriteTable(all_db, "predRel2", predRel)

dbDisconnect(all_db)
```


```{r compare to old relevance predictions}
all_db <- RSQLite::dbConnect(RSQLite::SQLite(),
                    here::here("data","raw-data","sql-databases", "all_tables_v1.sqlite"), 
                                create=FALSE)
...
ggplot(test, aes(relevance_mean1, relevance_mean2))+
  geom_point()+
...
test %>%
  ...
  ggplot(aes(x=relevance_mean1)) +
  ggtitle("supplemental coding articles")+
  geom_histogram()



## Explore the changes in inclusions/exclusions between the datasets
...
changedScreens %>%
  filter(include_screen == 0 & sample_screen == "test list")
changedScreens %>% filter(sysrev_id == "91813131")  
screens %>% filter(sysrev_id == "91813131")
...
nrow(predRelMeta) # check number of rows is correct -- 268452
