...
```{r assess performance to predict to known values and compare to old just screening relevance predictions}
...
## use the test list screens to classify articles into branch
library(metagear)

testListBranch <- screens %>% 
  filter(sample_screen == "test list" & include_screen == 1) %>%
  select(analysis_id, sysrev_id, title, abstract)

temp <- effort_distribute(testListBranch, 
                          initialize = T, save_split = T,
                          reviewers = c("Devi"), effort= c(100),
                  directory = here::here("data/raw-data/test-list"))

buttonOptions <- c("Mitigate","Natural","Societal","Other")
keyOptions <- c("m","n","s","o")

metagear::abstract_screener(
  here::here("data/raw-data/test-list/effort_Devi.csv"),
  aReviewer = "Devi", abstractColumnName = "abstract", titleColumnName = "title",
  theButtons = buttonOptions, keyBindingToButtons = keyOptions)

testListBranch2 <- read.csv(here::here("data/raw-data/test-list/effort_Devi.csv"))

testListBranch <- testListBranch %>%
  left_join(testListBranch2 %>% select(sysrev_id, INCLUDE)) %>%
  rename(ORO_branch = INCLUDE)
...
test <- screens %>%
  ...
  mutate(include_screen = factor(include_screen, 
                                 levels = c(1,0), labels = c("include","exclude")),
         ORO_branch = factor(ORO_branch, levels = c("Mitigate","Natural","Societal","Other"))) %>%
...
test %>%
  reshape2::melt(measure.vars = c("relevance_mean1", "relevance_mean2",
                                  "Mitigation_mean","Nature_mean","Societal_mean")) %>% 
  ggplot()+
...
performanceTabs <- colSums(predRelSummary[,c("TP","FP","FN")])
performanceTabs
#  TP  FP  FN 
# 925 172  31
predBranchSummary <- test
predBranchSummary$maxPred <- apply(
  test[,c("Mitigation_mean", "Nature_mean", "Societal_mean")], 1, max)
...
# how many inclusions and exclusions overall?
predBranchBinary <- data.frame(
  analysis_id = predBranch$analysis_id,
  Mitigation = ifelse(0.5 <= predBranch$Mitigation_mean, 1, 0),
  Nature = ifelse(0.5 <= predBranch$Nature_mean, 1, 0),
  Societal = ifelse(0.5 <= predBranch$Societal_mean, 1, 0)
)
predBranchBinary$Combined <- rowSums(predBranchBinary[,2:4])
predBranchBinary$Combined[predBranchBinary$Combined>1]<-1

colSums(predBranchBinary[,2:5])
