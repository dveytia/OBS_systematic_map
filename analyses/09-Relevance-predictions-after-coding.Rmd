...
# Add predictions to sqlite database -- do not run
...
df <- screens %>%
  ...
  rename(relevance_mean0 = relevance_mean) %>%
  left_join(predRel1 %>% select(analysis_id, relevance_mean), by="analysis_id") %>% 
...
predRel0Summary <-  df %>%
  mutate(predIncl = ifelse(0.5 <= relevance_mean1, 1, 0),
         TP = ifelse(0.5 <= relevance_mean0 & include_screen == "include", 1,0),
         FP = ifelse(0.5 <= relevance_mean0 & include_screen == "exclude", 1,0),
         FN = ifelse(relevance_mean0 < 0.5 & include_screen == "include", 1,0)) 
...
df %>%
  select(include_screen, relevance_mean0, relevance_mean1, relevance_mean2, branchMaxPred) %>%
  reshape2::melt(measure.vars = c("relevance_mean0","relevance_mean1", "relevance_mean2","branchMaxPred")) %>% 
  mutate(variable = factor(variable, labels = modelLevels))%>%
  ggplot()+
...
# facet by sampling strategy as well -- patterns remain consistent
df %>%
  select(include_screen, sample_screen,relevance_mean0, relevance_mean1, relevance_mean2, branchMaxPred) %>%
  reshape2::melt(measure.vars = c("relevance_mean0", "relevance_mean1", "relevance_mean2","branchMaxPred")) %>% 
  ggplot()+
  geom_hline(yintercept = 0.5, linetype="dashed", col="red")+
  geom_violin(aes(x=variable, y=value, fill = variable), trim=T)+
...
df %>%
  ...
  reshape2::melt(measure.vars = c("relevance_mean0","relevance_mean1",
                                  "relevance_mean2","branchMaxPred"),
                 variable.name = "model")%>% 
  mutate(model = factor(model, labels = modelLevels))%>%
...
branchDf <- df %>%
  select(-c(relevance_mean0, relevance_mean1, relevance_mean2)) %>%
...
screens %>%
  ...
  summarise(n=n())
reshape2::acast(screens, include_screen~sample_screen)
...
changedScreensRelevance <- changedScreens %>%
  ...
  left_join(df %>% select(duplicate_id, relevance_mean2, relevance_mean1, relevance_mean0, branchMaxPred)) %>%
  reshape2::melt(measure.vars = c("relevance_mean2","relevance_mean1",
                                  "relevance_mean0","branchMaxPred"),
                 variable.name = "model",value.name="relevance")%>%
  mutate(model = factor(model, 
                            levels = c(paste0("relevance_mean",0:2),"branchMaxPred"),
                            labels = paste0(c("* ","","* ",""), modelLevels))) 


ggplot(changedScreensRelevance, aes(x=model, y=relevance, fill=model)) +
  geom_violin()+
  labs(y= "Predicted relevance",
       caption = "amongst the articles that were included at screening and excluded at coding\n * These articles were not seen by these models")+
  theme(
