...
oroBranchArray <- do.call(abind::abind,
                          list(
                            plyr::alply(
                              responseArray[,dimnames(responseArray)[[2]][
                                grep("oro_type",dimnames(responseArray)[[2]])],],
                                3, oroBranchFn),
                            along = 3
                          ))
...
```{r try kraemer 1980 kappa designed for multiple choice responses}
...
source(here::here("R/kraemerKappa.R"))
...
for(v in 1:length(variables)){
  ...
  if(length(dim(temp2)) == 2){
    ...
    # If the variable has multiple choice, then use the kraemer (1980) method
    kappaResults$kappaMethod[v] <- "kraemer (1980)"
   ...
   kappa <- kraemerKappa(temp2[-unique(excl[,1]),,]) 
   ...
   x <- kmeans(dist(temp2Mat),2)
    ...
    if(min(table(x$cluster)) == 1){ # if there is one reviewer in a cluster (outlier)
       clust <- names(which.min(table(x$cluster)))
       outlier <- which(x$cluster == names(which.min(table(x$cluster))))
       kappa <- kraemerKappa(temp2[,,-outlier])
...
fun <- function(x){
  maxes <- which(table(x) == max(table(x)))
  if(length(maxes)==1){
    out <- as.numeric(names(which.max(table(x))))
  }else{
    out <- 1 # if the 0 and 1 is tied, put 1
  }
}
apply(responseArray[4:5,7:15,], 1:2, FUN=fun)
...
noORO <- which(rowSums(responseDf[,grep("oro_type", colnames(responseDf))], na.rm=T) == 0)
if(length(noORO) >0){
  View(responseArray[intersect(incl, noORO)[1],grep("oro_type", colnames(responseDf)),])
...
# screening decisions
responseDf$include_code[responseDf$sysrev_id == 91759652] <- 0 # set to exclusion b/c it was set to exclusion before
...
# check there are no columns getting dropped
...
anyDuplicated(codebookCombined$duplicate_id)

```


```{r tabulate all coding results}
source(here::here("R/tabulateCoding.R"))

tabulated_variables <- tabulateCoding(
  codebookCombined,
  c("sysrev_id","analysis_id","duplicate_id","title","abstract","keywords","year","coder_1","coder_2")
)
...
write.table(
  ...
  file = here::here("data","derived-data","coding","manual-coding", "all-coding-format-distilBERT.txt"),
...
# save tabulations
sink(here::here("outputs","coding","tabulated_coding_simplified.txt"))
print(tabulated_variables)
sink()
...
```{r use the tabulated coded variables to determine F1 score calculation method}
## load
all_db <- RSQLite::dbConnect(RSQLite::SQLite(),
                      here::here("data","raw-data","sql-databases", "all_tables_v1.sqlite"), 
                                 create=FALSE)
...
## Tabulate
source(here::here("R/tabulateCoding.R"))
tabulated_variables <- tabulateCoding(codebookCombinedSimplified,
               idCols = c("sysrev_id","analysis_id","duplicate_id","title",
                          "abstract","keywords","year","coder_1","coder_2"))


## Explore
varMeans <- unlist(lapply(tabulated_variables, function(x) mean(x, na.rm=T)))
varMedian <- unlist(lapply(tabulated_variables, function(x) quantile(x, probs = 0.5, na.rm=T)))
meanMinusMedian <- varMeans-varMedian

hist(meanMinusMedian,breaks = 20)

# which variables are in the top 75% where one value is very low
vars <- variables_to_tab[which(meanMinusMedian > quantile(meanMinusMedian, probs=0.75))]

for(i in 1:length(vars)){
  print(vars[i])
  print(tabulated_variables[[vars[i]]])
}

tabulated_variables$time_period # low value = all pre-industrials, combine with modern
tabulated_variables$data_scale_spatial # low value = micro --> combine with small

# variables where one value is very high
variables_to_tab[which(meanMinusMedian < 0)]
# impact ncp is because one value that is any
tabulated_variables$impact_ncp
# economic sector is very high on renewable_energy. 
tabulated_variables$economic_sector
```


```{r simplify codebook even more}
idCols <- c("sysrev_id","analysis_id","duplicate_id","title","abstract","keywords","year","coder_1","coder_2")
...
# reformat
source(here::here("R","simplifyCodebook2.R"))
codebookCombinedSimplifiedMore <- simplifyCodebook2(codebookCombinedSimplified,
                                                    idCols,
                                                    combineAlgaeCultivation = FALSE)


# tabulate
source(here::here("R/tabulateCoding.R"))
tabulated_variables <- tabulateCoding(codebookCombinedSimplifiedMore,
               idCols = c("sysrev_id","analysis_id","duplicate_id","title",
                          "abstract","keywords","year","coder_1","coder_2"))

sink(here::here("outputs","coding","tabulated_coding_simplifiedMore.txt"))
print(tabulated_variables)
sink()


# write to database
RSQLite::dbWriteTable(all_db, 
                      "allCodingSimplifiedVariablesMore", 
                      codebookCombinedSimplifiedMore, overwrite=TRUE) 

RSQLite::dbDisconnect(all_db)

# write to txt file for Vicky
write.table(
  codebookCombinedSimplifiedMore,
  file = here::here("data","derived-data","coding","manual-coding", "all-coding-format-distilBERT-simplifiedMore.txt"),
  row.names=F, col.names=TRUE, sep='\t', quote=FALSE
)
```

```{r compare counts amongst clusters of variable priorities}
## Load & format
...
# clusters
varPriority <- readxl::read_excel(
  here::here("data","derived-data","coding","variablePriorities.xlsx")
)
varPriority <- varPriority[,c("Variable name","Cluster","Cluster priority")]
colnames(varPriority) <- c("variable","cluster","clusterPriority")
varPriority <- na.omit(varPriority)
clusterPriorityLookup <- varPriority[!duplicated(varPriority[,2:3]), 2:3]
clusterPriorityLookup <- clusterPriorityLookup %>% arrange(clusterPriority)
clusterLev <- clusterPriorityLookup$cluster
#clusterLev <- clusterLev[-1] # remove intervention_0 for now because it's ORO_branch



## Data frame of the tabulation
for(i in 1:length(tabulated_variables)){
  if(is.null(names(tabulated_variables[[i]]))){
    values = names(tabulated_variables)[i]
  }else{
    values = names(tabulated_variables[[i]])
  }
  
  temp <- data.frame(
    variable = rep(names(tabulated_variables)[i], length(tabulated_variables[[i]])),
    value = values,
    counts = tabulated_variables[[i]]
  )
  rownames(temp) <- NULL
  if(i==1){
    clusterCounts <- temp
  }else{
    clusterCounts <- rbind(clusterCounts, temp)
  }
}

# Format columns
clusterCountsOld <- clusterCounts
clusterCounts <- inner_join(clusterCounts, varPriority, by="variable")
clusterCountsOld[!(clusterCountsOld$variable %in% clusterCounts$variable),] # check missing vars
rm(clusterCountsOld)
clusterCounts$variable.value <- paste(clusterCounts$variable,
                                      clusterCounts$value, sep=".")
clusterCounts <- clusterCounts %>% filter(variable != "oro_branch")
clusterCounts$cluster <- factor(clusterCounts$cluster, levels = clusterLev)
clusterCounts <- clusterCounts%>%
  arrange(cluster)%>%
  mutate(order = row_number())

write.csv(clusterCounts, 
          here::here("outputs/coding/tabulatedVariableClusters.csv"))


## Summarise
clusterCountsSummary <- clusterCounts %>%
  group_by(cluster)%>%
  summarise(
    mean = mean(counts),
    median = quantile(counts, probs=0.5),
    low_q = quantile(counts, probs=0.25),
    high_q = quantile(counts, probs=0.75),
    variance = var(counts),
    cv = sd(counts)/mean(counts),
    nLabels = n()
  )

write.csv(clusterCountsSummary, 
          here::here("outputs/coding/tabulatedVariableClustersSummary.csv"))

## Plots 

# ggplot(clusterCounts, aes(x=cluster, y=counts, fill = cluster, col = cluster))+
...
# ggplot(clusterCounts, aes(x = reorder(variable, order), y=counts, fill = cluster))+
#   geom_col()+
#   theme(
#     axis.text.x = element_text(angle=45, hjust=1)
#   )
...
require(gtable)
clusters <- unique(clusterCounts$cluster)
nplots <- 4 # number of plots/page
page <- seq(1,length(clusters), by=nplots) # number of pages
# create a list item to hold the plots
ggps <- vector("list", length(page))
ggps <- lapply(ggps, function(x) vector("list", nplots))

for(c in 1:length(clusters)){
  # identify page to write to
  p <- page[which(page-c <=0)]
  pNum <- p[which.max(p)]
  p <- which.max(p)
  # filter data
  temp <- clusterCounts %>%
    filter(cluster == clusters[c]) %>%
  ...
  # plot
  ggps[[p]][[c-pNum+1]] <- ggplot(temp, aes(x=valueOrder, y=counts, fill=variable))+
    geom_col()+
    scale_fill_discrete(name = paste(gsub("_"," ", clusters[c])))+
    scale_x_discrete(limits = levels(temp$valueOrder), 
                     labels = temp$value[order(temp$valueOrder)])+
    theme(
...
pdf(here::here("figures/additional/codingVariableClustering.pdf"), onefile = TRUE,
    width = 10, height = 10)
for(i in 1:length(ggps)){
  ggTemp <- ggps[[i]]
  ggTemp <- ggTemp[!sapply(ggTemp, is.null)]

  do.call("ggarrange", ggTemp)
}
...
# Alternate screening dataset - remove coding exclusions entirely
...
all_db <- RSQLite::dbConnect(RSQLite::SQLite(),
                    here::here("data","raw-data","sql-databases", "all_tables_v1.sqlite"), 
                                create=FALSE)
dedups <- tbl(all_db, "uniquerefs") %>% collect()
allScreens <- tbl(all_db, "allScreen_afterCoding") %>% collect()
# get screen results before coding
load(here::here("data","derived-data","screening","screened-records","screen_results_merged.RData"))



# Remove articles that were originally classed as include then changed
# to exclude after coding stage completely from the screening dataset
# essentially, keep exclusions from initial screening, and inclusions from after coding
screenExclID <- screen_results_merged$sysrev_id[which(screen_results_merged$include_screen == FALSE)]
codeInclID <- allScreens$sysrev_id[which(allScreens$include_screen == 1)]


allScreens2 <- allScreens[which(allScreens$sysrev_id %in% c(screenExclID, codeInclID)),]

exclInd <- which(allScreens2$sample_screen == "supplemental coding" &
        allScreens2$include_screen == 0)
allScreens2 <- allScreens2[-exclInd,]

# check that no exclusions are sampled by supplemental coding
allScreens2 %>%
  group_by(include_screen, sample_screen)%>%
  summarise(n=n())
# check that the number of exclusions == number of exclusions from initial
sum(allScreens2$include_screen == 0) == sum(!(screen_results_merged$include_screen))

```

```{r write list of new test list articles}
...
```{r write for vicky, eval = FALSE}

## Write for Vicky
write.table(
  allScreens2,
  file = here::here("data","derived-data","screening","screened-records","all-screen-results_screenExcl-codeIncl.txt"),
  row.names=F, col.names=TRUE, sep='\t', quote=FALSE
)
...
# Exploration for model fitting

```{r}
# load in table of coding decisions
codebookCombinedSimplifiedMore <-read.delim(here::here("data","derived-data","coding","manual-coding", "all-coding-format-distilBERT-simplifiedMore.txt"))

df <- subset(
  codebookCombinedSimplifiedMore, 
  select = -c(sysrev_id,analysis_id,title,abstract,keywords,year,coder_1,coder_2)
)

```

## Cramer's V for nominal variable association

useful links: https://towardsdatascience.com/the-search-for-categorical-correlation-a1cf7f1888c9

Calculate the Cramer's V statistic (analagous to correlation coefficient for nominal variables) to see if there are associations between variables. This could indicate that they might be ammenable to clustering. Currently having trouble with getting high F1 scores for ecosystem_type, impact_nature and impact_ncp, so we will focus on those.

```{r}
library(rcompanion)

variables = colnames(df)[2:ncol(df)]
variableNames = gsub("[.]"," - ", variables)
variableNames = gsub("_"," ", variableNames)
variableNames = gsub("  "," ", variableNames)

cramerMat <- matrix(nrow=ncol(df)-1, ncol = ncol(df)-1, 
                    dimnames = list(var1 = variableNames,
                                    var2 = variableNames))
for(i in 1:nrow(cramerMat))
  for(j in 1:ncol(cramerMat))
    cramerMat[i,j] <- cramerV(as.factor(df[,i+1]), as.factor(df[,j+1]))


cramerMat[upper.tri(cramerMat, diag = TRUE)] <- NA

image(t(cramerMat))

cramerMat2 <- reshape2::melt(cramerMat)
...
cramerMat2 %>%
  ...
  labs(
    fill = "Cramer's\nV",
    caption = "showing Cramer's V > 0.5")+
  theme(
...
ggsave(here::here("figures/supplementary/codedVariablesCramersVAssociation.pdf"),width = 7,
       height = 5.5, units="in")
```
