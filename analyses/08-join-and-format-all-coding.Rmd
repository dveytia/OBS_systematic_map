...
ggps <- vector("list", length(clusters))

for(c in 1:length(clusters)){
  temp <- clusterCounts %>%
    filter(cluster == clusters[c])
  ggps[[c]] <- ggplot(temp, aes(x=value, y=counts, fill=variable))+
    geom_col()+
    scale_fill_discrete(name = paste(clusters[c]))+
    theme(
      axis.text.x = element_text(angle=45, hjust=1),
      axis.title.x = element_blank()
    )
  ggps[[c]] <- ggplotGrob(ggps[[c]])
}


gridExtra::grid.arrange(ggps[[1]],ggps[[2]],ggps[[3]],ggps[[4]], nrow=2)
gridExtra::grid.arrange(ggps[[5]],ggps[[6]],ggps[[7]],ggps[[8]], nrow=2)
gridExtra::grid.arrange(ggps[[9]],ggps[[10]], nrow=2)
```
