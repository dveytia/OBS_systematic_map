...
#   year <- unlist(strsplit(responseDf$duplicate_id[i],"[.]"))[1]
#   my_text_sub <- subset(my_text, year == year)
#   match <- stringdist::amatch(tolower(responseDf$title[i]), 
#                                    tolower(my_text_sub$title), method = "osa", maxDist = 5, nomatch = NA)
#   if(is.na(matches[i])){
#     match <- stringdist::amatch(responseDf$duplicate_id[i], my_text_sub$duplicate_id, method = "osa", maxDist = 5, nomatch = NA)
#   }
#   
#   matches[i] <- my_text_sub$duplicate_id[match]
# }
# 
# responseDf2 <- cbind(
#   data.frame(duplicate_id = matches),
#   responseDf[,-c(1:3)]
# )
# responseDf2 <- right_join(
#   my_text[,c("analysis_id","duplicate_id","title","abstract","keywords","year")], 
#   responseDf2, by="duplicate_id")


#View(responseDf2[which(responseDf2$duplicate_id == "1985.2160"),]) # check this is the right label, should be ENVIRONMENTAL ENHANCEMENT OF THE OCEANS BY INC
