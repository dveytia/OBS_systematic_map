...
all_db <- RSQLite::dbConnect(RSQLite::SQLite(),
                      here::here("data","raw-data","sql-databases", "all_tables_v1.sqlite"), 
                                 create=FALSE)
src_dbi(all_db)
RSQLite::dbWriteTable(all_db, "allCodingSimplifiedVariables", codebookCombined, overwrite=TRUE)
...
# save(supplementalScreen, my_text, matches,
#      file = here::here("data","derived-data","coding","keyword-matches-supplemental-coding","supplementalScreenMatches.RData"))

# just load from saved file -- it's quicker
load(here::here("data","derived-data","coding","keyword-matches-supplemental-coding","supplementalScreenMatches.RData"))
sum(is.na(matches))
...
all_db <- RSQLite::dbConnect(RSQLite::SQLite(),
                      here::here("data","raw-data","sql-databases", "all_tables_v1.sqlite"), 
                                 create=FALSE)
src_dbi(all_db)
RSQLite::dbWriteTable(all_db, "allScreen_afterCoding", allScreens)
...
code_effort_summary
#    DV JGP AC LB YS FV
# 1 659  32 93 82 33 43

code_effort_summary <- reshape2::melt(code_effort_summary)

mean(code_effort_summary$value) # 
sd(code_effort_summary$value) # 
median(code_effort_summary$value) # 
