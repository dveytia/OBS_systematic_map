...
# for the array of responses, find the most common answer for each variable.

# list all the variables
variableValues <- dimnames(responseArray)[[2]]
...
# change the "1" in the data frame to the name of the value
temp <- responseArray
...
# returns 1 if there is any agreement between a vector and the common response
agreeFn <- function(y, commonResponse){
  agreements <- intersect(commonResponse, y)
  if(1 <= length(agreements)){
    return(1)
  }else{
    0
  }
...
for(v in 1:length(variables)){
  ...
  temp2 <- temp[,grep(variables[v],variableValues),]
  ...
  dim(agreement) # should be 20 x 6 (n articles x n coders)

  # calculate the kappa statistic
  kappa <- irr::kappam.fleiss(agreement)
