...
test_list <- litsearchr::import_results(
  file = here::here("data", "derived-data", "screen_test_list", 
                   "test-list_indexed_in_WOS_2023-01-19.ris")
)
...
# de-duplicate again because some duplicates got through
# screenDups <- revtools::screen_duplicates(screen_results)
...
nrow(screen_results2) <= sum(duplicated(matchingDf$titleMatch))
...
# these are all from the test list -- 
...
```{r test list check}
missedTestList <- screen_results_merged %>%
