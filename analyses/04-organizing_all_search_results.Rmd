...
```{r set up folders filepaths containing records}
...
for(d in 1:length(dbFolderNames)){ 
  ...
  database <- names(database_resultsFolders)[d]
  
  # names of all the folders holding results
  resultsFolders <- names(database_resultsFolders[[d]])
  ...
  for(s in 1:length(resultsFolders)){ 
    ...
    for(f in 1:length(files)){
      ...
      # write to database
      db <- RSQLite::dbConnect(RSQLite::SQLite(), dbname = sqlite_all_ref_file)
      dbWriteTable(db, "refs", tempDf, append=appendl, overwrite = overwritel)
      dbDisconnect(db)
