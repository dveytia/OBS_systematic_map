...
for(i in 1:nrow(write2pdf)){
  ...
  filename <- paste(write2pdf$sysrev_id[i], 
                    gsub(" ","-",
                         stringr::str_trim(
                      gsub('[[:punct:] ]+',' ',
                           tolower(
...
length(dedupInfo$title_year_dup_id)-sum(duplicated(dedupInfo$title_year_dup_id))

# save as a .sqlite
db <- RSQLite::dbConnect(RSQLite::SQLite(), 
                         dbname = here::here(resultsPath, "deduplication-files", "title_dedup_ids.sqlite"))
dbWriteTable(db, "title_dedup_ids", tempDf, append=FALSE, overwrite = TRUE)
dbDisconnect(db)

```

```{r Assign the duplicate ID to each record}

# Open connection to the database
dbcon <- dbConnect(RSQLite::SQLite(), sqlite_all_ref_file_version)

# note this takes a while to run 
# add a column with the dedup id to the sqlite database
dbExecute(dbcon, "UPDATE refs SET duplicate_id = :duplicate_id where analysis_id = :analysis_id",
          params=data.frame(duplicate_id=dedupInfo$title_year_dup_id,
                            analysis_id=dedupInfo$analysis_id))

# check the first few rows to see that it has worked
dbGetQuery(dbcon, "SELECT * FROM refs LIMIT 3")
...
dedup_db <- dbConnect(RSQLite::SQLite(), here::here(resultsPath, "deduplication-files", "title_dedup_ids.sqlite"),
                      create=FALSE)
dedups <- tbl(dedup_db, "deduplication-files")



## join the datasets with their duplicate info
# do left join so that all the references are maintained
allrefs_join <- allrefs %>%
  left_join(dedup_db) # keeps all obs in x. Could use collect() to get all the data but I fear it would crash
...
allrefs_join %>%
  ...
  summarise(n_unique_refs = distinct(duplicate_id))
...
## Close connections
dbDisconnect(allrefs_db)
dbDisconnnect(dedup_db)

```

```{r summarise the duplicates - old}

# open the connection
db <- src_sqlite(sqlite_all_ref_file_version, create=FALSE)
allrefs <- tbl(db, "refs")

## Summarise numbers of duplicates
# by database source
allrefs %>%
  group_by(source_database) %>%
  summarise(n_unique_refs = distinct(duplicate_id))
...
deduplicate the database using Rossi

```{r examine the deduplicated database}
# open the connection
db <- src_sqlite(here::here(resultsPath, "sql-databases", "deduplicated-refs.sqlite"), create=FALSE)
dedupRefs <- tbl(db, "dedupRefs")

## Summarise

# summarize how many unique references in total
dedupRefs %>% summarise(n())


# number of records by database
dedupRefs %>% group_by(source_database) %>% summarise(n())
