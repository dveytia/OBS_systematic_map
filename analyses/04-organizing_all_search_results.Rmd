...
# scopus results
scopus_resultsDir <- here::here("data","raw-data","Scopus_downloads")
...
sqlite_all_wos_file <- file.path(wos_resultsDir,"sql-databases","sample-wos-refs.sqlite")

# file to for the sql database
sqlite_scopus_file <- file.path(scopus_resultsDir,"sql-databases","sample-scopus-refs.sqlite")
table_name <- "scopusrefs"
...
# De-duplicate all the WOS and Scopus records
...
```{r set up folders containing records}

## use inputs to extract file paths
wos_resultsFolders <- dir(wos_resultsDir)[grep("23", dir(wos_resultsDir))]
scopus_resultsFolders <- dir(scopus_resultsDir)[grep("23", dir(scopus_resultsDir))]

# make sure the folders are in the same order
wos_resultsFolders <- wos_resultsFolders[order(wos_resultsFolders)]
scopus_resultsFolders <- scopus_resultsFolders[order(scopus_resultsFolders)]

# get names for the different blocks -- should be the same for both
blockNames <- unlist(lapply(str_split(wos_resultsFolders,"_"), function(x) x[[1]]))

# get the file names and store in a list
results_files <- list()
results_files$WOS <- vector("list", length(wos_resultsFolders))
names(results_files$WOS) <- blockNames
results_files$Scopus <- vector("list", length(scopus_resultsFolders))
names(results_files$Scopus) <- blockNames

for(f in 1:length(wos_resultsFolders)){
  results_files$WOS[[f]] <- dir(file.path(wos_resultsDir,wos_resultsFolders[f]))
  results_files$Scopus[[f]] <- dir(file.path(scopus_resultsDir,scopus_resultsFolders[f]))
}
...
wosFormat <- revtools::read_bibliography(
  file.path(wos_resultsDir, wos_resultsFolders[1], results_files$WOS[[1]][1]))
scopusFormat <- revtools::read_bibliography(
  file.path(scopus_resultsDir, scopus_resultsFolders[1], results_files$Scopus[[1]][1]))
