...
screenColnamesOut <- c("sysrev_id","title","source_title","author", "test_list_article","screener",
                       "include_screen", "Double_Blind","filtered_screen","multiple_oro_screen", 
...
for(f in 1:length(screenedRecs_files)){
  ...
  # filter to only resolved records
  tempDf <- subset(tempDf, Status == "consistent")
  ...
  tempDf$Double_Blind <- grepl("double-blind", screenedRecs_files[f])
...
screen_results$test_list_article <- ifelse(is.na(screen_results$test_list_article), FALSE, TRUE)
screen_results$sample_screen <- ifelse(screen_results$test_list_article, "test list", "random")


# now reorder columns
screen_results$test_list_article <- NULL
screenColnamesOut2 <- screenColnamesOut
...
# check at least each screen result has a match
nrow(screen_results2) <= sum(duplicated(titleMatches))

# join match to the original dataframe
...
# for full record
sampleDf3 <- merge(sampleDf, matchingDf[which(matchingDf$source == "whole_ref"),])
sampleDf3$source <- NULL
View(sampleDf3[which(duplicated(sampleDf3$titleMatch)),]) # check any remaining duplicates
sampleDf3 <- sampleDf3[-which(duplicated(sampleDf3$titleMatch))[1:2]] # remove duplicates manuall
...
screen_results_merged <- merge(
  screen_results3, sampleDf3 %>% select(all_of(-c(removeCols))), by=c("titleMatch"), all.y = FALSE
)

# check there are no duplicated doi
sum(is.na(screen_results_merged$doi))

# clean confusing columns
screen_results_merged <- screen_results_merged[,-c("titleMatch", "n_duplicates")]
...
lastCols <- colnames(write2pdf)[which(!(colnames(write2pdf) %in% firstCols))]
...
```{r summarise screening information}
...
for(i in 1:nrow(write2pdf)){
  ...
  render("my_text.Rmd", pdf_document(),
         output_file = here::here(
           "data","derived-data","coding","coding-pdfs", filename
         ))
