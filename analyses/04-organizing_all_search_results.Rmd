...
```{r}
load(file.path(resultsPath, "deduplication-files","list-results.RData"))

nCols <- lapply(results, function(x) ncol(x))

mod.rbind <- function(x){
  if(nrow(x)==1){
    return(X)
  }else{
    next
...
```{r examine the deduplicated database to check that it has the number of entries expected}
# open the connection
db <- src_sqlite(sqlite_unique_ref_file, create=FALSE)
dedupRefs <- tbl(db, sqlite_unique_ref_table_name)

## Summarise

# summarize how many unique references in total
dedupRefs %>% summarise(n=n())
