...
Heatmap of ORO type by marine system -- maybe as an infographic? 




Heatmap of ORO type and outcome
...
# get mangrove
mangrove_uuid <- get_uuid(name = "Rhizophora mangle", n=5)
mangrove_img <- pick_phylopic(name = "Rhizophora mangle", n = 1)
mangrove_img <- get_phylopic("bacc6a59-80d2-461c-92d4-83cabc91cc39")
save_phylopic(img=mangrove_img,
              path = here::here("figures/phylopics/mangrove.svg"))

# Get coral
coral_img <- get_phylopic("32eeaa29-2e90-40b5-92fb-eb8d7fcab9ab")
save_phylopic(img = coral_img,
              path = here::here("figures/phylopics/coral.svg"))

# Get macroalgae
macro_img <- get_phylopic("9acf11fa-89dd-4a56-b3da-8665c406c6f5")
save_phylopic(img = macro_img,
...
#seagrass_img <- get_phylopic("2534e10f-ea8e-436e-a8c8-13ac045eb867")
#save_phylopic(img = seagrass_img,
...
predAdapt <- readPredictions(boundaries = "mean", variable = "adapt_to_threat")
...
predAdapt <- predAdapt %>%
  mutate(outcome = recode_factor(adapt_to_threat,
                                 Human = "Adapt: Humans", Natural = "Adapt: Nature",
                                 Both = "Adapt: Humans & nature"),
         outcome_category = paste("Adaptation")) %>%
  mutate(outcome = as.character(outcome),
        outcome_category = as.character(outcome_category))%>%
  select(-c(adapt_to_threat))
...
oro_eco_outcome_ggp <- tempTab %>%
  ...
  annotation_raster(marsh_ras, 0.5, 1.5, -4, -0)+
  annotation_raster(mangrove_ras, 1.5, 2.5, -4, -0)+
  annotation_raster(coral_ras, 2.5, 3.5, -4, -0)+
  annotation_raster(macro_ras, 3.5, 4.5, -4, -0)+
  annotation_raster(seagrass_ras, 4.5, 5.5, -4, -0)+
  coord_cartesian(ylim = c(-3.5, length(levels(tempTab$oro_any))), clip = 'off')+
  labs(
