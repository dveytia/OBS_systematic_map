...
Three way heatmaps of oro, ecosystem, outcome
...
predAdapt <- predAdapt %>%
  mutate(outcome = recode_factor(adapt_to_threat,
                                 Human = "Human adapt", Natural = "Nature adapt",
                                 Both = "Human & nature adapt"),
...
temp <- predOROAny %>%
  filter(0.5 <= mean) %>%
  select(-c(mean))%>%
  full_join(predEcos %>% filter(0.5 <= mean) %>%select(-c(mean)), by = "analysis_id") %>%
  full_join(predOutcome%>% filter(0.5 <= mean) %>%select(-c(mean)), by = "analysis_id") %>%
  filter(!is.na(oro_any)) # remove "other" oro_type

temp2 <- temp %>%
  group_by(ecosystem_type, oro_any, outcome_category) %>%
  na.omit() %>%
  summarise(z = n()) %>% View
  filter(ecosystem_type != "Other" & outcome_category == "Mitigation") %>%
  reshape2::acast(ecosystem_type ~ oro_any, value.var = "z") %>%
  as.matrix()
temp2[is.na(temp2)] <- 0

# Need to fill in missing combinations
# Need to add colourbars on side for the grouping of ORO type
library("gplots")
hm1 <- heatmaply(
  x = temp2,
  dendrogram = "none",
  #main = "a.",
  margins = c(50,50,30, 30),
  xlab = "ORO type",
  ylab = "Ecosystem type",
  key.title = "N articles",
  heatmap_layers = list(ggtitle("a.")),
  plot_method = "ggplot",
  labCol = c(oroAnyColsLabels[1:6],"Conservation"),
  #labRow = ,
  row_side_colors = colorspace::rainbow_hcl(nrow(temp2))
...
ggheatmap(
  ...
  labCol = c(oroAnyColsLabels[1:6],"Conservation"),
  #labRow = ,
  col_side_colors = colorspace::rainbow_hcl(ncol(temp2))
...
subplot(hm1, hm1, margin = .1, shareY=TRUE, shareX = TRUE, nrows = 2)
