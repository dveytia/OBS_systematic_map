...
research_area_tab <- research_area_tab %>%
  ...
  mutate(research_area = ifelse(research_area %in% low_research_areas$research_area,
                                as.character(research_area), "Other")) %>% 
  mutate(research_area = factor(research_area, levels = c(low_research_areas$research_area, "Other"))) 
...
# Extract legend from ggplot and make it's own grob
...
Chloropleth Map of First Author Affiliation

to annual-trends-research-area plot -- panel a Add label of total number of articles for each type (lower, upper)

Annual trends in # articles one panel for each branch and then all the numbers of articles for each type
