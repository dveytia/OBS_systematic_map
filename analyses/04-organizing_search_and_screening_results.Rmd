...
# open the connection
db <- RSQLite::dbConnect(RSQLite::SQLite(),sqlite_unique_ref_file, create=FALSE)
src_dbi(db)
...
stringsCombn <- t(combn(strings,2))
stringsCombn <- as.data.frame(stringsCombn)
stringsCombn$duplicates <- rep(NA, nrow(stringsCombn))
colnames(stringsCombn) <- c("string1","string2","duplicates")

dedupRefsAll <- dedupRefs %>% collect() 
dedupRefsAll <- dedupRefsAll[grepl(" | ", dedupRefsAll$search_substring),]

for(i in 1:nrow(stringsCombn)){
  stringsCombn$duplicates[i] <- sum(grepl(stringsCombn[i,1], dedupRefsAll$search_substring) & 
        grepl(stringsCombn[i,2],dedupRefsAll$search_substring), na.rm=T)
}

stringsCombn %>% tidyr::spread(key = "string1", value = "duplicates", fill = 0)

#      string2 General Mitigation Nature Renewable
# 1 Mitigation    1954          0      0         0
# 2     Nature    7160      18995      0      3663
# 3  Renewable     513       6326      0         0
# 4   Societal    3764       2586  10379      1026
