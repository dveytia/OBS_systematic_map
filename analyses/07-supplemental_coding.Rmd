...
subGroups <- c("Acidification", "Assisted_evo")
...
# order dataset according to spreadsheet of terms
screens_all2 <- cbind(screens_expr, screens_swd)
screens_all2 <- screens_all2[,match(gsub(" ","_", nlp_search_terms$Term[which(nlp_search_terms$Group %in% subGroups)]), colnames(screens_all2))]

# make sure everything is just a y/n response, to presence/absence
screens_all2 <- ifelse(1 <= screens_all2, 1, 0)

screens_all2 <- cbind(data.frame(analysis_id = my_text$analysis_id),screens_all2)


saveRDS(screens_all2,
        here::here("data","derived-data","coding","keyword-matches-supplemental-coding","keywordMatches2.rds"))
...
for(g in 1:length(groups)){
  ...
  # across all the terms, I only care if there is a match to any term
  if(length(colInd)>1){
    tempTab <- rowSums(screens_all2[,colInd], na.rm = TRUE)
  }else{
    tempTab <- screens_all2[,colInd]
  }
  tempTab[tempTab > 1] <- 1
