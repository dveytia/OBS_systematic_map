...
```{r source functions}
functionsToSource <- c("clean_string.R", "screen.R","tokenization.R")
...
predRel_db <- dbConnect(SQLite(), 
                        dbname = here::here("data","raw-data","sql-databases","relevance-predictions.sqlite"),create = FALSE)
...
my_text <- left_join(relRefs, dedups %>% 
                       select(analysis_id,title, author, year, 
                              journal, abstract, keywords, research_areas), 
                     by = "analysis_id", copy=TRUE)
my_text <- my_text %>%
  collect()
