...
nlp_search_terms <- readRDS(here::here("data","derived-data","coding","keyword-matches-supplemental-coding","keyword-search-tokens-format.rds"))


## Separate out words to screen
...
## Screen for keywords (takes a while to run)
...
```{r just screen for acidification and evolutionary adaptation}
## Read in data
my_text <- readRDS(here::here("data","derived-data","coding","keyword-matches-supplemental-coding","screeningText.rds"))
...
# limit to only some groups
subGroups <- c("Acidification", "Assisted_evo", "Ocean_PP","Algae_cultivation","Weathering")

# separate out into single terms and expressions
single_words <- nlp_search_terms$Term[which(nlp_search_terms$Term.type == "single" &
                                              nlp_search_terms$Group %in% subGroups)]
expressions <- nlp_search_terms$Term[which(nlp_search_terms$Term.type == "expression" &
                                              nlp_search_terms$Group %in% subGroups)]
# name them by their corresponding group
names(single_words) <- nlp_search_terms$Group[which(nlp_search_terms$Term.type == "single" &
...
# order dataset according to spreadsheet of terms
screens_all3 <- cbind(screens_expr, screens_swd)
screens_all3 <- screens_all3[,match(gsub(" ","_", nlp_search_terms$Term[which(nlp_search_terms$Group %in% subGroups)]), colnames(screens_all3))]

# make sure everything is just a y/n response, to presence/absence
screens_all3 <- ifelse(1 <= screens_all3, 1, 0)

screens_all3 <- cbind(data.frame(analysis_id = my_text$analysis_id),screens_all3)


saveRDS(screens_all3,
        here::here("data","derived-data","coding","keyword-matches-supplemental-coding","keywordMatches3.rds"))
...
for(g in 1:length(groups)){
  ...
  colInd <- which(colnames(screens_all2) %in% 
                    gsub(" ","_", nlp_search_terms$Term[which(nlp_search_terms$Group == groups[g])]))
...
for(g in 1:length(groups)){
  ...
  # which columns (terms) in the screens_all2 matrix apply to a given group
  colInd <- which(colnames(screens_all3) %in% 
                    gsub(" ","_", nlp_search_terms$Term[which(nlp_search_terms$Group == groups[g])]))
  
  # across all the terms, I only care if there is a match to any term
  if(length(colInd)>1){
    tempTab <- rowSums(screens_all3[,colInd], na.rm = TRUE)
  }else{
    tempTab <- screens_all3[,colInd]
  }
  tempTab[tempTab > 1] <- 1
...
screens_all3 <- readRDS(here::here("data","derived-data","coding","keyword-matches-supplemental-coding","keywordMatches3.rds"))
...
# write.csv(screenDf, 
#           here::here("data","derived-data","coding","keyword-matches-supplemental-coding","efficiencyMatches2Screen.csv"))
...
# to avoid not saving, do in chunks
starts <- seq(1, nrow(screenDf), by=20)
ends <- starts-1

i=2 # which chunk I'm at

# To avoid error, I have to rename analysis_id column as label
revtools::screen_abstracts(
  x = screenDf[starts[i]:ends[i+1],] %>% 
...
## I've got enough examples of bleaching, so now remove those keyword matches
screenDf <- tabMatches( # tablulate number of matches retrieved for each article
  screenMatrix = screens_all,
  group = "Temperature",
  keywordGroupLookupTable = nlp_search_terms,
...
# filter out those I've already screened
alreadyScreenedDir <- here::here("data","derived-data","coding","keyword-matches-supplemental-coding","screening")
alreadyScreenedFiles <- dir(alreadyScreenedDir)

for(f in 1:length(alreadyScreenedFiles)){
  temp <- read.csv(file.path(alreadyScreenedDir, alreadyScreenedFiles[f]))
  if(f==1){
    alreadyScreened <- temp
  }else{
    alreadyScreened <- rbind(alreadyScreened, temp)
  }
}

screenDf <-  subset(screenDf, !(duplicate_id %in% alreadyScreened$label))
...
```{r screen biodiversity articles}

## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant

# tablulate number of matches retrieved for each article
# remove coral cover as a keyword because I already have enough coral articles from temperature screening
screenDf <- tabMatches( 
  screenMatrix = screens_all,
  group = "Biodiversity",
  keywordGroupLookupTable = nlp_search_terms,colExclude = "coral_cover")
...
```{r screen acidification articles}
## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant

# tablulate number of matches retrieved for each article
# remove coral cover as a keyword because I already have enough coral articles from temperature screening
screenDf <- tabMatches( 
  screenMatrix = screens_all2,
  group = "Acidification",
  keywordGroupLookupTable = nlp_search_terms)
...
```{r screen assisted evolution articles}
## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant

# tablulate number of matches retrieved for each article
# remove coral cover as a keyword because I already have enough coral articles from temperature screening
screenDf <- tabMatches( 
  screenMatrix = screens_all3,
  group = "Assisted_evo",
  keywordGroupLookupTable = nlp_search_terms)
...
```{r screen ocean primary production articles}
## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant
...
```{r screen algae cultivation articles}
## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant
...
```{r screen alkalinity or weathering articles}
## Identify and sort articles by number of matches -- this way hopefully prioritizing most relevant

# tablulate number of matches retrieved for each article
# remove lime as keyword because it was popping up matches for wind energy, etc
screenDf1 <- tabMatches( 
  screenMatrix = screens_all3,
  group = "Weathering",
  keywordGroupLookupTable = nlp_search_terms)
...
screenDf1 <- subset(screenDf1, n_matches > 0) # subset to only the matches
screenDf <- subset(screenDf, n_matches > 0) # subset to only the matches

screenDf1 <- merge.data.frame( # join with metadata
  screenDf1, my_text %>% select(-text), by = "analysis_id", all.x = TRUE, all.y = FALSE
)
screenDf <- merge.data.frame( # join with metadata
  screenDf, my_text %>% select(-text), by = "analysis_id", all.x = TRUE, all.y = FALSE
)
...
screenDf1 <- screenDf1 %>% # sort dataframe in order of likely decreasing relevance
  arrange(desc(n_matches), desc(relevance_mean))
screenDf <- screenDf %>% # sort dataframe in order of likely decreasing relevance
  arrange(desc(n_matches), desc(relevance_mean))
...
screenDf1 <-  subset(screenDf1, !(duplicate_id %in% alreadyScreened$label))
screenDf <-  subset(screenDf, !(duplicate_id %in% alreadyScreened$label))
...
revtools::screen_abstracts(
  x= screenDf1[which(screenDf1$duplicate_id %in% c("2022.2982","2013.2715","2022.2020","2022.4772")),] %>% 
    select(title, author, year, journal, abstract, research_areas, duplicate_id) %>%
    rename(label = duplicate_id)
)
...
Note that excel changes the formatting of duplciate id (e.g. 2022.2020 -> 2022.202). So merge based on title if I can
