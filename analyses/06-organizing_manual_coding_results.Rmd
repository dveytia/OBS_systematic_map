...
```{r increase energy efficiency}
library(litsearchr)
library(dplyr)

temp <- responseDf %>%
  filter(climate_threat.Temperature == 1 | 
           oro_type.M_Increase_efficiency == 1 | 
           oro_type.N_Human_assisted_evolution ==1 |
           adapt_to_threat.Natural == 1)
...
for(v in 1:length(vars)){
  ...
  minN <- 1 # minimum number of words in a keyword
...
Overall, the terms are too generic -- will need to design simplified ones from the search string instead.
